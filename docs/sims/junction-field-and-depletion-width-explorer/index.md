...
description: A MicroSim applying Poisson's equation to the depletion approximation, showing depletion charge density, junction electric field, and electrostatic potential stacked and aligned, with x_p, x_n, W, E_max, and V_bi all marked directly on the charts and a draggable position marker
...
<iframe src="main.html" height="990px" width="100%" scrolling="auto" style="border:none; border-radius:8px; overflow:hidden;"></iframe>
</div>
...
This MicroSim solves Poisson's equation for a silicon step junction under the depletion approximation, showing three aligned, stacked charts: the depletion charge density \(\rho(x)\), the resulting triangular junction electric field \(E(x)\), and the electrostatic potential \(\psi(x)\) obtained by integrating the field. Dashed reference lines mark the metallurgical junction \(x=0\), the depletion edges \(-x_p\) and \(x_n\), the total width \(W\), the peak field \(E_{max}\), and the built-in potential \(V_{bi}\) directly on the charts. A draggable marker — spanning the full plotted range, not just the depletion region — reads exact values at any position, and the doping sliders update every quantity live.
</p>
...
<li style="margin-bottom: 0.6rem; line-height: 1.75; color: #333; padding-left: 1.4rem; text-indent: -1.4rem;"><span style="color: #5A3EED; font-weight: 700; margin-right: 0.4rem;">&#9679;</span> <strong>Symmetric / p+–n / p–n+ presets</strong> for instantly comparing doping-asymmetry effects</li>
<li style="margin-bottom: 0.6rem; line-height: 1.75; color: #333; padding-left: 1.4rem; text-indent: -1.4rem;"><span style="color: #5A3EED; font-weight: 700; margin-right: 0.4rem;">&#9679;</span> <strong>\(x_p\), \(x_n\), \(W\), \(E_{max}\), and \(V_{bi}\) marked directly</strong> on the charts, not just listed numerically</li>
<li style="margin-bottom: 0.6rem; line-height: 1.75; color: #333; padding-left: 1.4rem; text-indent: -1.4rem;"><span style="color: #5A3EED; font-weight: 700; margin-right: 0.4rem;">&#9679;</span> <strong>Full-range position marker</strong> synchronized across all three plots, including outside the depletion region</li>
